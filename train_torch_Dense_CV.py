...
def create_ResNet50(output_dim=1):
    """
    build ResNet50
    """
...
def create_DenseNet169(output_dim=1):
    """
    build DenseNet169
    """
...
def train(max_epoch=MAX_EPOCH):
    for idx in range(5):
        ...
        net = DenseNet169_plus(pretrained=True)
        net.to(DEVICE)
        ...
        torch.save(net.state_dict(), "DenseNet169plus_cv{}_best_epoch{}.pth".format(idx, best_epoch))
        print("-" * 30, "\nFinish Training cv{}, {} Epoch\nThe Best is epoch {}\n".format(idx, MAX_EPOCH, best_epoch))


def predict():
    for idx in range(5):
        net = DenseNet169_plus(pretrained=False)
        net.to("cpu")

        preds = []
        weight_path = glob.glob("DenseNet169plus_cv{}*pth".format(idx))
        ...
        if os.path.exists("./output"):
            sub.to_csv("./output/{}_Dense169plus_cv{}_proba.csv".format(time.strftime("%m%d"), idx))
        else:
            os.mkdir("./output")
            sub.to_csv("./output/{}_Dense169plus_cv{}_proba.csv".format(time.strftime("%m%d"), idx))
        print("File Saved!")
...
# model = models.densenet169(pretrained=True)
# NET = DenseNet169(model)
# NET.to(DEVICE)

# NET_test = DenseNet169_plus(pretrained=False)

# -------------------- Model2 Densenet169 Plus--------------------#
