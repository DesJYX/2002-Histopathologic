...
TRANS_test = get_x_trans_origin()

kf = KFold(n_splits=5, shuffle=True, random_state=SEED)
KF_Dataset = {}

for cnt, (train_idx, val_idx) in enumerate(kf.split(TRAIN_CSV)):
    print(cnt)
    train_part = TRAIN_CSV.iloc[train_idx]
    val_part = TRAIN_CSV.iloc[val_idx]
...
class SE_ResNext_50(nn.Module):
    ...
    def __init__(self, ):
        ...
        self.Linear_layer = nn.Linear(2048 + 2, 32)
    ...
    def forward(self, x):
        x = self.model_layer(x)  # [-1, 2048, 7, 7]

        batch = x.shape[0]  # [-1, 2048, 7, 7]
        max_pool, _ = torch.max(x, 1, keepdim=True)  # [-1, 1, 7, 7]
        avg_pool = torch.mean(x, 1, keepdim=True)  # [-1, 1, 7, 7]

        conc = torch.cat((x, max_pool, avg_pool), 1)  # [-1, 2050, 7, 7]
        conc = self.Avg_Pooling(conc)  # [-1, 2050, 1, 1]
        conc = conc.view(batch, -1)  # [-1, 2050]
        conc = self.Linear_layer(conc)  # [-1, 32]
...
class SE_ResNext_101(nn.Module):

    def __init__(self, ):
        super(SE_ResNext_101, self).__init__()
...
def train(max_epoch=MAX_EPOCH):
    for idx in range(5):
        ...
        optimizer = optim.SGD(net.parameters(), lr=1e-4)  # 定义优化器和目标函数
        criterion = nn.BCEWithLogitsLoss()  # 自带sigmoid
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=1e-4, max_lr=0.0004, step_size_up=7000)
        ...
        torch.save(net.state_dict(), "SE_ResNext_50_cv{}_best_epoch{}.pth".format(idx, best_epoch))
...
def predict():
    for idx in range(5):
        ...
        if os.path.exists("./output"):
            sub.to_csv("./output/{}_SE_ResNext_50_cv{}_proba.csv".format(time.strftime("%m%d"), idx))
        else:
            os.mkdir("./output")
            sub.to_csv("./output/{}_SE_ResNext_50_cv{}_proba.csv".format(time.strftime("%m%d"), idx))
        print("File Saved!")
...
# tta_file = glob.glob("output/0320*")
...
# df_tta.to_csv("output/TTA_0320_SE_ResNext_CV5.csv", index=False)
#
